...
const UserRepsModal: React.FC<RepDetailsModalProps> = ({
  open,
  onClose,
  id,
}) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { data } = await axiosT.get(`/admin/report/user/${id}`);
        setRep(data);
      } catch (e) {
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Box sx={style}>
        <span className="font-bold text-2xl">{t("reporedtUser")}</span>
