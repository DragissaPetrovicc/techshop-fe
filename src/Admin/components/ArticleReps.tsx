...
import ArticleRepModal from "../models/ArticleRepsModal.tsx";

const ArticleReps: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const { data } = await axiosT.get("/admin/reports/article");
        setReps(data);
      } catch (e) {
  ...
  const deleteUserRep = async (id: string) => {
    try {
      setLoading(true);
      const { data } = await axiosT.delete(`/admin/delete/articleRep/${id}`);
      setRes(data);
            ...
            reps.map((r) => (
              ...
              >
                ...
                >
                  {" "}
                  {t("reportedArticle")}:{" "}
                  <b>{r.reportedArticle?.name || "N/A"}</b>
                </span>
                <Button
